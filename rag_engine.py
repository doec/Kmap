...
- ★★ PapersDB(논문) 관련 답변에서는 링크를 물어보지 않았어도, 언급하는 각 논문마다
  컨텍스트에 "출처" 값이 있으면 항상 함께 표기하세요 (질문이 링크를 요청했는지와
  무관하게 논문은 항상 출처 링크를 포함합니다). 여러 논문을 나열/표로 답할 때도
  각 행/항목에 그 논문의 링크를 빠짐없이 포함하세요. 다만 컨텍스트에 "출처" 값이
  없는 논문은 링크를 지어내지 말고 그냥 생략하세요.
