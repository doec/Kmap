...
답변은 아래처럼 데이터셋마다 별도의 마크다운 소제목(###)으로 나누고,
각 소제목 아래에 그 데이터셋의 내용을 정리하세요.
{heading_list}
- 표로 요약하라는 요청이면, 각 소제목 아래에 그 데이터셋의 표를 각각 만드세요.
- 가능하면 검색된 데이터셋을 모두 언급하되, 질문과의 관련성이 낮거나 내용이 거의
  없는 데이터셋은 소제목 자체를 생략해도 됩니다(억지로 채우지 마세요). 관련성이
  뚜렷한 데이터셋은 빠뜨리지 마세요.
- ★★ 데이터셋별 섹션을 전부 작성한 뒤, 맨 마지막에 반드시 "### 종합 요약" 소제목을
  추가하고, 위 모든 데이터셋의 결과를 한데 모아 종합적으로 검토한 요약을 작성하세요.
  이건 각 섹션을 짧게 재요약하는 게 아니라, 데이터셋을 가로질러 공통된 결론/추세,
  서로 보완되거나 상충되는 내용, 질문에 대한 최종 답을 종합적으로 판단해서 쓰는
  것입니다. 이 섹션은 절대 생략하지 마세요 — 여러 데이터셋 결과를 사용자가 각각
  따로 읽고 스스로 종합해야 하는 부담을 줄이는 것이 목적입니다."""
