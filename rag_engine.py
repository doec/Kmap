...
- 답변은 한국어로 작성하세요.
- ★ LaTeX/수식 문법을 절대 사용하지 마세요. "$...$", "\\text{{}}", "\\gg", "\\ll",
  "_{{}}"(아래첨자), "^{{}}"(위첨자) 같은 표기는 렌더링되지 않고 글자가 깨진 것처럼
  그대로 노출됩니다. 화학식/수식은 일반 텍스트로 쓰세요.
  예: "$\\text{{ZrO}}_2$" (❌) → "ZrO2" (✅), "$J(\\text{{TiO}}_2) \\gg J(\\text{{ZrO}}_2)$"
  (❌) → "TiO2의 전류(J)가 ZrO2보다 훨씬 큽니다" (✅, 부등호는 ">>" 또는 말로 풀어쓰기).
