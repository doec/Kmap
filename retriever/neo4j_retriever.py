    ...
    def _build_type_colors(self) -> dict:
        ...
        try:
            with self.driver.session() as session:
                ...
                # ★ Paper/Report(구 Document)는 메타 노드 타입이라 서브그래프에
                #   그려지지 않으므로 범례(legend)에서도 제외해 엔티티 타입만 유지한다.
                types = [
                    r['t'] for r in session.run(
                        "MATCH (n) WHERE n.type IS NOT NULL "
                        "RETURN DISTINCT n.type AS t ORDER BY t"
                    ) if r['t'] and r['t'] not in ('Unknown', 'Paper', 'Report')
                ]
    ...
    def generate_rag_result_graph(self, node_names: list[str],
        ...
        try:
            with self.driver.session() as session:
                ...
                #   - 메타 노드(Paper/Report, 구 Document)는 이름이 doc_id 라서
                #     보통 $names 에 안 잡히지만, 방어적으로 레이블로도 제외한다.
                query = """
                    MATCH (s)-[r]->(o)
                    WHERE s.name IN $names AND o.name IN $names
                      AND NOT type(r) IN ['FROM_PAPER', 'FROM_DOC']
                      AND NOT s:Paper AND NOT s:Report
                      AND NOT o:Paper AND NOT o:Report
                """
                if dataset and dataset != 'All':
